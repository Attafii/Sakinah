...
// Ensure AIGuide is available in this service worker scope
try {
    importScripts && importScripts('ai.js');
} catch (e) {
    console.warn('Background: Could not import ai.js via importScripts:', e);
}
...
class SakinahBackground {
    ...
    async loadSettings() {
        try {
            const settings = await chrome.storage.sync.get({
                ...
                notificationFrequency: null, // Fallback for old key
                customTimes: [],
                quietStart: '22:00',
                quietEnd: '07:00',
    ...
    async showDailyAyahNotification() {
        ...
        let explanation = '';
        try {
            if (typeof AIGuide !== 'undefined' && AIGuide && typeof AIGuide.explainAyah === 'function') {
                explanation = await AIGuide.explainAyah(ayah);
            } else {
